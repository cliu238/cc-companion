# Usage API Rate Limit Experiment — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Build a standalone Python script that experimentally determines the safe polling interval for the Anthropic OAuth usage API.

**Architecture:** Single async Python script with 3 sequential experiment phases (steady-state → recovery → burst). A `CircuitBreaker` class tracks 429s and terminates on dangerous status codes. All results written to append-only log file with checkpoint support for resume.

**Tech Stack:** Python 3.12+, uv, httpx (async HTTP), asyncio
...
**Files:**
- Create: `scripts/usage_rate_limit_experiment/pyproject.toml`
- Create: `scripts/usage_rate_limit_experiment/experiment.py`

**Step 1: Create pyproject.toml**

```toml
[project]
name = "usage-rate-limit-experiment"
version = "0.1.0"
requires-python = ">=3.12"
dependencies = ["httpx>=0.28"]

[project.scripts]
run-experiment = "experiment:main"
```

**Step 2: Create experiment.py with CLI skeleton**
...
Run: `cd scripts/usage_rate_limit_experiment && uv run python experiment.py --help`
Expected: help text showing `--enable-burst`, `--resume`, `--output-dir`

**Step 4: Commit**

```bash
git add scripts/usage_rate_limit_experiment/
git commit -m "feat: scaffold usage API rate limit experiment (#2)"
```

---

### Task 2: Token Reader & HTTP Client

**Files:**
- Modify: `scripts/usage_rate_limit_experiment/experiment.py`

**Step 1: Write test for token reading**

Add to bottom of `experiment.py`:
...
**Step 2: Implement read_token()**

Add above `_test_read_token`:
...
**Step 3: Verify token reading**

Run: `cd scripts/usage_rate_limit_experiment && uv run python experiment.py test`
Expected: `Token found: xxxxxxxx...xxxx`

**Step 4: Add make_request() function**
...
git commit -m "feat: add token reader and HTTP client for experiment (#2)"
```

---

### Task 3: Logger & Circuit Breaker
...
```bash
git add scripts/usage_rate_limit_experiment/experiment.py
git commit -m "feat: add Logger and CircuitBreaker for experiment (#2)"
```

---

### Task 4: Checkpoint & Resume

**Files:**
- Modify: `scripts/usage_rate_limit_experiment/experiment.py`

**Step 1: Implement Checkpoint class**
...
```bash
git add scripts/usage_rate_limit_experiment/experiment.py
git commit -m "feat: add Checkpoint for experiment resume (#2)"
```

---

### Task 5: Phase 1 — Steady-State

**Files:**
- Modify: `scripts/usage_rate_limit_experiment/experiment.py`

**Step 1: Implement steady_state phase**
...
### Task 6: Phase 2 — Recovery

**Files:**
- Modify: `scripts/usage_rate_limit_experiment/experiment.py`

**Step 1: Implement recovery phase**
...
```bash
git add scripts/usage_rate_limit_experiment/experiment.py
git commit -m "feat: implement recovery phase (#2)"
```

---

### Task 7: Phase 3 — Burst (opt-in)

**Files:**
- Modify: `scripts/usage_rate_limit_experiment/experiment.py`

**Step 1: Implement burst phase**
...
```bash
git add scripts/usage_rate_limit_experiment/experiment.py
git commit -m "feat: implement burst phase (#2)"
```
...
**Step 1: Wire up the run() function**

Replace the placeholder `run()` function:
...
**Step 2: Run a dry test (no actual API call yet, just verify it starts)**

Run: `cd scripts/usage_rate_limit_experiment && uv run python experiment.py --output-dir /tmp/test_experiment`
Expected: Prints token info, starts Phase 1, makes first request (this will hit the real API — one request is safe)

**Step 3: Commit**

```bash
git add scripts/usage_rate_limit_experiment/experiment.py
git commit -m "feat: wire up main orchestrator for experiment (#2)"
```

---

### Task 9: Final Integration Test & Docs

**Files:**
- Modify: `scripts/usage_rate_limit_experiment/experiment.py`

**Step 1: Add a `--dry-run` mode for testing without hitting the API**

In `make_request()`, add at the top:

```python
DRY_RUN = False  # Set via CLI arg
...
Run: `cd scripts/usage_rate_limit_experiment && uv run python experiment.py --dry-run --output-dir /tmp/test_dry`
Expected: Runs all phases quickly with simulated responses, writes log and summary files

**Step 3: Add README to script directory**

Create: `scripts/usage_rate_limit_experiment/README.md`
...
```bash
git add scripts/usage_rate_limit_experiment/
git commit -m "feat: add dry-run mode and README for experiment (#2)"
```
